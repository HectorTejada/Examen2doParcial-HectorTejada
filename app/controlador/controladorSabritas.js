...
function actualizar(req,res) {
    ...
    sabritasObj.save().then(sabritasAlta => {
        res.status(200).send({message:'Los datos se actualizaron correctamente'});
...
function eliminar(req,res) {
    ...
    req.body.sabritas[0].remove().then(sabritasEliminar => {
        res.status(200).send({message:'La informacion se eiminó correctamente'.sabritasEliminar});
